...
namespace PointBlank.Game.Data.Managers
{
    public static class AccountManager
    {
            ...
            try
            {
                using (NpgsqlConnection npgsqlConnection = SqlConnection.getInstance().conn())
                {
                    ...
                    while (npgsqlDataReader.Read())
                    {
                        ...
                        acc.LastFreeBonus = npgsqlDataReader.IsDBNull(72) ? DateTime.Now : npgsqlDataReader.GetDateTime(72);
                        AddAccount(acc);
                    }
                    command.Dispose();
                    npgsqlDataReader.Close();
        ...
        public static void reloadMegaPhone(Account player)
        {
            using (NpgsqlConnection npgsqlConnection = SqlConnection.getInstance().conn())
            {
                NpgsqlCommand command = npgsqlConnection.CreateCommand();
                npgsqlConnection.Open();
                command.CommandText = "SELECT * FROM megaphone_messages";
                NpgsqlDataReader npgsqlDataReader = command.ExecuteReader();
                while (npgsqlDataReader.Read())
                {
                    player.SendPacket(new PROTOCOL_BASE_MEGAPHONE_ACK(player.player_name, npgsqlDataReader.GetString(3)));
                }
                command.Dispose();
